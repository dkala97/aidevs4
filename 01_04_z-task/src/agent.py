...
async def _run_tool(mcp_client: McpClient, tool_call: dict[str, Any]) -> dict[str, Any]:
    name = tool_call["name"]
    ...
    try:
        result = (
            await execute_native_tool(name, arguments)
            if is_native_tool(name)
            else await mcp_client.call_tool(name, arguments)
        )
...
async def run(
    ...
    *,
    ...
    tools = [*mcp_client.mcp_tools_to_openai(mcp_tools), *native_tools]
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        results = []
        for tool_call in tool_calls:
            results.append(await _run_tool(mcp_client, tool_call))
