...
VISION_MODEL = resolve_model_for_provider(os.getenv("TASK_VISION_MODEL", "gpt-5.2"))
...
Use MCP tools dynamically discovered from the connected server.
Use the native tool understand_image whenever important information may be present in scans, screenshots, tables, photos of text, or other document images.

Rules:
1. Read the available task materials first.
2. Do not invent missing facts.
3. Prefer MCP tools for files and structured operations.
4. Use understand_image only for image-based content.
5. Finish the task end-to-end and provide a concise final answer.
""".strip()

DEFAULT_QUERY = """Read the task materials in dane/ and any related files exposed by MCP.
If any important information exists in image files, use understand_image.
Work out the final task result autonomously and return the answer with a short justification.
""".strip()